...
def read_imgs_parallel(WSI_object,coords,patch_level,patch_size,first_mag,**kwargs):
    # print(patch_level)
    ...
    if first_mag==40:
        #resize from 512 to 256
        imgs = np.array([result.convert('RGB').resize((int(patch_size/2),int(patch_size/2))) for result in results if result is not None])
    else:
        imgs = np.array([result.convert('RGB') for result in results if result is not None])
...
def stitching(file_path, wsi_object, downscale=64,patch_size=256,patch_level=0):
    start = time.time()
    heatmap = StitchCoords(file_path, wsi_object, downscale=downscale, bg_color=(0, 0, 0), alpha=-1, draw_grid=False,patch_size=patch_size,patch_level=patch_level)
...
def fp_patching(WSI_object,num_selected_patches,img_path,patch_level,patch_size,first_mag,patch, **kwargs):
    ### Start Patch Timer
    start_time = time.time()
    ...
    if file_path and patch: #only extract images when patch is True
        patching_status = extract_imgs_from_coor(WSI_object, num_selected_patches, file_path,img_path,patch_level,patch_size,first_mag, **kwargs)
    patch_time_elapsed = time.time() - start_time
...
def extract_imgs_from_coor(WSI_object,num_selected_patches,coor_path,img_path,patch_level,patch_size,first_mag, **kwargs):
    ### Start extract Timer
    ...
    # todo: record the patch_level as well as selected or not
        ...
        select_imgs = read_imgs_parallel(WSI_object,selected_coors,patch_level,patch_size,first_mag,**kwargs)
...
def patch_transfer_single(i,process_stack,df,save_dir='/mnt/data/TUM_Slides_Patches/',
                      ...
                      stitch=True,
                      patch=True, auto_skip=True, process_list=None,num_selected_patches=500,coor_dir="/mnt/data/TUM_Slides_Patches/coor_h5/",annot_countour=False):
    ...
    #     df.loc[idx, 'status'] = 'already_exist'
    #     return None,0
    broken_ls = ['C3L-02622-28','C3L-05870-42']
    ...
    # Inialize WSI & get properties
    # full_path = os.path.join(source, slide)
    WSI_object = WholeSlideImage(slide)
    wsi = WSI_object.getOpenSlide()
    try:
        first_mag = wsi.properties['openslide.objective-power']
        if first_mag == '40':
            first_mag = 40
        else:
            first_mag = 20
        ...
        first_mag = 40
    print('first_mag: ', first_mag)

    # load contours
    if annot_countour:
        anno_file = os.path.join('/mnt/nfs03-R6/Yale_breast/Yale_HER2_cohort/Annotations', slide_id + '.xml')
        wsi_object.initXML(anno_file, 'Region', 'Vertex')
    ...
    if os.path.isfile(coor_h5_path): # already exists such coord h5 files
        ...
        patching_status,h5_file_path, patch_time_elapsed = fp_patching(WSI_object=WSI_object, num_selected_patches=num_selected_patches,img_path = img_patch_path,first_mag=first_mag,patch=patch, **current_patch_params, )
        ...
        if stitch:
            ...
            if os.path.isfile(file_path):
                heatmap, stitch_time_elapsed = stitching(file_path, WSI_object, downscale=64,patch_size=patch_size,patch_level=patch_level)
...
parser.add_argument('--coor_dir', default="/mnt/nfs03-R6/HEST_jingsong/clam_20/coor/",
...
parser.add_argument('--patch_size', type=int, default=256,
                    help='patch_size')
parser.add_argument('--num_selected_patches', type=int, default=99999,
                    help='num of each selected slides')
parser.add_argument('--patch', default=False, action='store_true')
parser.add_argument('--seg', default=True,action='store_true')
parser.add_argument('--stitch', default=False, action='store_true')
parser.add_argument('--no_auto_skip', default=True, action='store_false')
parser.add_argument('--save_dir', type=str,default='/mnt/nfs03-R6/HEST_jingsong/',
...
parser.add_argument('--tumour_contour', default=False,
                    help='whether to load the contour of annotated tumour or not')
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.coor_dir):
        print('create coor dir')
        os.makedirs(args.coor_dir, exist_ok=True)
    ...
    elif 'CPTAC' in args.save_dir:
        slides = sorted(os.listdir(args.save_dir + 'svs/'))
        # print(slides)
        slides = [os.path.join(args.save_dir + 'svs/', slide) for slide in slides if
                  os.path.isfile(os.path.join(args.save_dir + 'svs/', slide)) and slide.endswith('.svs')]

    elif 'TCGA' in args.save_dir:
        slides_folder_ls = sorted(os.listdir(args.save_dir))
        for slide_folder in slides_folder_ls:
            sub_folder_ls = sorted(os.listdir(os.path.join(save_dir,slide_folder)))
            slides = [os.path.join(args.save_dir, slide_folder, slide) for slide in sub_folder_ls if
                      os.path.isfile(os.path.join(args.save_dir, slide_folder, slide)) and slide.endswith('.svs')]
    elif 'Yale' in args.save_dir:
        # print(args.save_dir)
        slides = sorted(os.listdir(os.path.join(args.save_dir,'SVS')))
        #print(slides)
        slides = [os.path.join(args.save_dir,'SVS', slide) for slide in slides if
                  os.path.isfile(os.path.join(args.save_dir,'SVS', slide)) and slide.endswith('.svs')]
        slides = sorted(slides)
    elif 'patch_selection' in args.save_dir:
        slides = sorted(os.listdir(os.path.join(args.save_dir, 'slides')))
        slides = [os.path.join(args.save_dir,'slides', slide) for slide in slides if
                  os.path.isfile(os.path.join(args.save_dir,'slides', slide)) and slide.endswith('.svs')]
        slides = sorted(slides)
        # slides = slides[12:14]
    elif 'HEST' in args.save_dir:
        slides = sorted(os.listdir(os.path.join(args.save_dir,'wsis')))
        #print(slides)
        slides = [os.path.join(args.save_dir,'wsis', slide) for slide in slides if
                  os.path.isfile(os.path.join(args.save_dir,'wsis', slide)) and slide.endswith('.tif')]
        slides = sorted(slides)
    ...
    # print(slides)
    print('Total number of slides: '+str(len(slides)))


    if process_list is None:
        df = initialize_df(slides, seg_params, filter_params, vis_params, patch_params, save_patches=True)
        process_list = os.path.join(args.save_dir, 'clam_20', "process_list_autogen.csv")
    ...
    # for i in range(int(total / 5)):
        ...
        #               patch=args.patch, auto_skip=args.no_auto_skip, process_list=process_list,num_selected_patches=args.num_selected_patches,coor_dir=args.coor_dir,annot_countour=args.tumour_contour)

        h5_file_path,t,df = patch_transfer_single(i,process_stack,df=df, save_dir=patch_save_dir,
                        patch_size=args.patch_size, step_size=args.step_size, custom_downsample=1,
                        seg_params=parameters['seg_params'],
                        filter_params=parameters['filter_params'],
                        vis_params=parameters['vis_params'],
                        patch_params=parameters['patch_params'],
                        seg=args.seg, use_default_params=False, save_mask=args.save_mask,save_h5=args.save_patch,
                        stitch=args.stitch,patch_level=args.patch_level,
                        patch=args.patch, auto_skip=args.no_auto_skip, process_list=process_list,num_selected_patches=args.num_selected_patches,coor_dir=args.coor_dir,annot_countour=args.tumour_contour)
