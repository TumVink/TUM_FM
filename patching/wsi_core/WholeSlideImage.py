...
class WholeSlideImage(object):
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256,
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                print(self.holes_tissue[cont_idx])
                if not self.isInContours(cont_check_fn, (int(x), int(y)), self.holes_tissue[cont_idx],
    ...
    def isInContours(cont_check_fn, pt, holes=None, patch_size=256):
        ...
        if isinstance(pt, (np.ndarray, np.generic)):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256,mp_mode='patch', **kwargs):
        save_path_hdf5 = os.path.join(save_path, str(self.name) + '.h5')
        # print('patch_level:' + str(patch_level))
        # print('patch_size:' + str(patch_size))
        # print('step_size:' + str(patch_size))
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict, len_result = self.process_contour(cont, self.holes_tissue[idx], patch_level,
                                                                     save_path, patch_size, step_size,mp_mode, **kwargs)
            n_patches += len_result
            if len(asset_dict) > 0:
                if init:
                    self.hdf5_file = save_hdf5(save_path_hdf5, asset_dict, attr_dict, mode='w')
                    init = False
                else:
                    self.hdf5_file = save_hdf5(save_path_hdf5, asset_dict, mode='a')
            # if n_patches >= 2*500:
            #     break

        return self.hdf5_file, n_patches


    def process_contours_not_save(self, num_selected_patches, save_path, patch_level=0, patch_size=256, step_size=256,
                                  **kwargs):
        save_path_hdf5 = os.path.join(save_path, str(self.name) + '.h5')
        print("Creating patches for: ", self.name, "...", )
        elapsed = time.time()
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict, len_result = self.process_contour(cont, self.holes_tissue[idx], patch_level,
                                                                     save_path, patch_size, step_size, **kwargs)
            ...
            if n_patches >= num_selected_patches:
                print('Reached the maximum number of patches: {}'.format(num_selected_patches))
                break

        return n_patches

    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256, mp_mode='patch',
        ...
        coord_candidates = np.array([x_coords.flatten(), y_coords.flatten()]).transpose().astype(int)
        # print(coord_candidates.shape)
        if mp_mode == 'patch':
        ...
        if len(results) > 1:
            ...
            return asset_dict, attr_dict, len(results)
