...
def fp_patching(WSI_object,num_selected_patches,img_path,patch_level,patch_size,first_mag, **kwargs):
    ### Start Patch Timer
    start_time = time.time()
...
def patch_transfer_single(i,process_stack,df,save_dir='/mnt/data/TUM_Slides_Patches/',
    ...
    if os.path.isfile(coor_h5_path): # already exists such coord h5 files
        ...
        if patch:
            ...
            patching_status,h5_file_path, patch_time_elapsed = fp_patching(WSI_object=WSI_object, num_selected_patches=num_selected_patches,img_path = img_patch_path,first_mag=first_mag, **current_patch_params, )
...
if __name__ == '__main__':
    ...
    if 'SLN' in args.save_dir:
    ...
    elif 'crc' in args.save_dir:
        df = pd.read_csv(
            '/mnt/nfs03-R6/TCGA/crc_msi_cls/csv_file/crc_msi_cls_magnification.csv')
        slides_basedir = '/mnt/nfs03-R6/TCGA/'
        slides = []
        for i in range(len(df)):
            slide_dir = os.path.join(slides_basedir,df.loc[i,'id'],df.loc[i,'filename'])
            if os.path.isfile(slide_dir):
                slides.append(slide_dir)
            else:
                print(slide_dir+' file not found')
                break
    elif 'UCEC' in args.save_dir:
        df = pd.read_csv(
            '/mnt/nfs03-R6/CPTAC/UCEC/survival_pred/cptac.csv')
        slides_basedir = '/mnt/nfs03-R6/CPTAC/UCEC/'
        slides = []
        for i in range(len(df)):
            slide_dir = os.path.join(slides_basedir, df.loc[i, 'slide_id']+ '.svs')
            if os.path.isfile(slide_dir):
                slides.append(slide_dir)
            else:
                print(slide_dir + ' file not found')
        ...
        slides_basedir = '/mnt/nfs03-R6/TCGA/'
        df = pd.read_csv(
            '/mnt/nfs03-R6/TCGA/survival_pred/TCGA.csv')
        for i in range(len(df)):
            slide_dir = os.path.join(slides_basedir,df.loc[i, 'id'], df.loc[i, 'slide_id']+ '.svs')
            if os.path.isfile(slide_dir):
                slides.append(slide_dir)
            else:
                print(slide_dir + ' file not found')
