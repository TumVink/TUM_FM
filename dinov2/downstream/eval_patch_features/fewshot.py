...
def eval_knn(
    ...
    labels_source = train_labels.cpu().detach().numpy()
    feats_query = test_feats
    labels_query = test_labels.cpu().detach().numpy()
    ...
    if average_feats:
        ...
        labels_proto = torch.Tensor(sorted(np.unique(labels_source))).to(feats_source.device)

    # SimpleShot Eval
    pw_dist = (feats_query[:, None] - feats_proto[None, :]).norm(dim=-1, p=2)
    # labels_query = torch.Tensor(labels_query).to(feats_source.device)
    labels_pred_proto = labels_proto[pw_dist.min(dim=1).indices]
    proto_metrics = get_eval_metrics(labels_query, labels_pred_proto.cpu().detach().numpy(), prefix="proto_")
    proto_dump = {
    ...
    # KNN Eval
    labels_source = torch.Tensor(labels_source).to(feats_source.device)
    knn = sklearn.neighbors.KNeighborsClassifier(n_neighbors=n_neighbors, n_jobs=num_workers)
    labels_pred_knn = knn.fit(feats_source.cpu().detach().numpy(), labels_source.cpu().detach().numpy()).predict(feats_query.cpu().detach().numpy())
